...
def test_randomize_check(testclient: TestClient, fresh_db, page: Page):
    assert testclient.get('/api/utils/randomize_checks/').status_code == status.HTTP_200_OK
    assert testclient.get('/api/utils/randomize_checks/?within=24').status_code == status.HTTP_200_OK
