...
def test_user_api(testclient: TestClient, fresh_db):
    ...
    # get
    resp = testclient.get(f'/user?idx={user1.id}')
    ...
    resp = testclient.post(f'/user/', user1.json(exclude_unset=True))
    assert resp.status_code == status.HTTP_200_OK
    ...
    # list
    resp = testclient.get(f'/user/list/')
    assert resp.status_code == status.HTTP_200_OK
    assert len(resp.json()) == 2

    # delete
    resp = testclient.delete(f'/user/?idx={user2.id}')
